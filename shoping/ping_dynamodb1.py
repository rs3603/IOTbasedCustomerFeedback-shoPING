...
DYNAMODB_TABLE_NAME = 'androidUser'
...
try:
        pingdata = Table.create('androidUser', schema=[HashKey('userNo',data_type=NUMBER)],connection = client_dynamo)
	count_dyn=0
	pingdata = Table('androidUser', schema = [HashKey('userNo',data_type=NUMBER)], connection = client_dynamo)
except:
...
def loop():
 ...
 # the sketch will turn on the LED attached to D13 on the board
	...
	count_dyn  = count_dyn+1;
        a = s.read(1)
	pingdata.put_item(data={'userNo':count_dyn,'firstName':str(count_dyn)+'-'+str(a),'lastName':str(time.time())},overwrite=True)
        time.sleep(1)
	print a
